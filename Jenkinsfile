...
EOF"""
          ...
          timeout(30) {
            sh """SSHPASS=${PASSWORD} sshpass -e ssh -tt -o StrictHostKeyChecking=no -o PubkeyAuthentication=no -p ${SSH_PORT} ${USERNAME}@${SSH_HOST} << EOF
cd ${installDir} && \
  temp-fixes-after-started.sh "${SSH_HOST}" "${USERNAME}" "${PASSWORD}" || { echo "[temp-fixes-after-started.sh] failed";}
echo "[temp-fixes-after-started.sh] succeeds"
EOF"""
...
ZOWE_INSTANCE_DIR=${zoweInstanceDir} \
SSH_HOST=${SSH_HOST} \
SSH_PORT=${SSH_PORT} \
SSH_USER=${USERNAME} \
SSH_PASSWD=${PASSWORD} \
