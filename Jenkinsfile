...
cd ${params.INSTALL_DIR} && \
  ...
  (./uninstall-zowe.sh \
    -i ${params.INSTALL_DIR} \
    -t ${params.ZOWE_ROOT_DIR} \
    -m ${params.PROCLIB_MEMBER} || { echo "[uninstall-zowe.sh] failed"; exit 0; }) && \
  echo "-----------------------------" && \
  ...
  ${smpePathZfs} \
  ${smpeFmid} \
  ${smpeRelfilePrefix} || { echo "[uninstall-SMPE-PAX.sh] failed"; exit 0; }) && \
