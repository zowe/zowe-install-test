...
cd ${params.INSTALL_DIR} && \
  ...
  echo "-----------------------------" && \
  (./uninstall-SMPE-PAX.sh ${smpeHlq} ${smpeHlqCsi} ${smpeHlqTzone} ${smpeHlqDzone} ${smpePathPrefix} ${params.INSTALL_DIR} ${smpePathZfs} ${smpeFmid} ${smpeRelfilePrefix} || { echo "[uninstall-SMPE-PAX.sh] failed"; exit 0; }) && \
